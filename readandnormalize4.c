...
{ 
    ...
    */
    ...
    eq_num += utf32to8bytes[1] << 16;
    eq_num += utf32to8bytes[2] << 8;
    eq_num += utf32to8bytes[3];

    //printf("%d", eq_num);

    if(eq_num <= 0b1111111){
        utf8bytes[nextIndex] = utf32to8bytes[3];
        nextIndex++;
    ...
    }else if(eq_num > 0b1111111111111111 && eq_num <= 0b111111111111111111111){
